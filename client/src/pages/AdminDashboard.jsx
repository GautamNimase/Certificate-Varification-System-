...
function AdminDashboard() {
  ...
  const fetchStudents = async () => {
    try {
      const response = await fetch(`${API_URL}/auth/users?role=student`, {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        {/* Message Alert */}
        {message.text && (
          <div className={`mb-6 p-4 rounded-lg ${message.type === 'success' ? 'bg-green-50 text-green-800 border border-green-bg-red-50 text-red-800200' : ' border border-red-200'}`}>
            {message.text}
          </div>
        )}
                      ...
                      {certificates.map(cert => (
                        <tr key={cert.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4">
                            ...
                            <p className="text-xs text-gray-500">{cert.certificate_hash.substring(0, 20)}...</p>
                          </td>
                          <td className="px-6 py-4">
                            <p className="text-gray-900">{cert.student?.name || 'N/A'}</p>
                            <p className="text-xs text-gray-500">{cert.student?.email || 'N/A'}</p>
                          </td>
